...
def polymerize(template, rules, steps):
    ...
    char_frequency = Counter(template)

    for position in range(1 ,len(template)):
        pair_curr =  template[position-1:position+1]
        pair_frequency[pair_curr] += 1

    for _ in range(steps):
        pair_frequency_step = Counter()
        for pair , frequency, in pair_frequency.items():
            if pair in rules:
                pair_frequency_step[pair[0] + rules[pair]] += frequency
                pair_frequency_step[rules[pair] + pair[1]] += frequency
                char_frequency[rules[pair]] += frequency
        pair_frequency = pair_frequency_step
