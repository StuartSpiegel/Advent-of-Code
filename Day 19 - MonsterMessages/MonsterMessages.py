# --- Day 19: Monster Messages --- You land in an airport surrounded by dense forest. As you walk to your high-speed
# train, the Elves at the Mythical Information Bureau contact you again. They think their satellite has collected an
# image of a sea monster! Unfortunately, the connection to the satellite is having problems, and many of the messages
...
# the rule 2: 1 3 | 3 1 means that to match rule 2, the text being checked must match rule 1 followed by rule 3 or it
...
#
# 0: 4 1 5 1: 2 3 | 3 2 2: 4 4 | 5 5 3: 4 5 | 5 4 4: "a" 5: "b" Here, because rule 4 matches a and rule 5 matches b,
# rule 2 matches two letters that are the same (aa or bb), and rule 3 matches two letters that are different (ab or
# ba).
#
...
# 5: "b"
#
# ababbb bababa abbbab aaabbb aaaabbb Your goal is to determine the number of messages that completely match rule 0.
# In the above example, ababbb and abbbab match, but bababa, aaabbb, and aaaabbb do not, producing the answer 2. The
# whole message must match all of rule 0; there can't be extra unmatched characters in the message. (For example,
# aaaabbb might appear to match rule 0 above, but it has an extra unmatched b on the end.)
#
...
def parse(f):
    ...
    for line in f:
        if line == '\n':
            break
        indent, rule = line.rstrip().split(': ')
        rules[int(indent)] = rule.replace('"', '')
...
def match(rules, messages):
    ...
    def group(indent):
        return '(?:' + ''.join(map(expand, rules[indent].split())) + ')'
