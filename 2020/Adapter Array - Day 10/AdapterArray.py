# --- Day 10: Adapter Array --- Patched into the aircraft's data port, you discover weather forecasts of a massive
# tropical storm. Before you can figure out whether it will impact your vacation plans, however, your device suddenly
# turns off!
#
# Its battery is dead.
#
...
# be the adapter rated 6 and then the adapter rated 7 (with difference of 1 and 1). The only adapter that works with
# the 7-jolt rated adapter is the one rated 10 jolts (difference of 3). From 10, the choices are 11 or 12; choose 11
# (difference of 1) and then 12 (difference of 1). After 12, only valid adapter has a rating of 15 (difference of 3),
# then 16 (difference of 1), then 19 (difference of 3). Finally, your device's built-in adapter is always 3 higher
# than the highest adapter, so its rating is 22 jolts (always a difference of 3). In this example, when using every
# adapter, there are 7 differences of 1 jolt and 5 differences of 3 jolts. Here is a larger example:
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
28
33
18
...
3
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% In this larger
# example, in a chain that uses all of the adapters, there are 22 differences of 1 jolt and 10 differences of 3 jolts.
#
...
numbers = [0]  # numbers array to hold the read in line as numerical diffs
...
ones, threes = 0, 0  # variables serving as counters for the number of one permutation groups and the number of three
# permutation groups

# brute force algorithm for Range checks
...
a1 = ones * threes  # the final answer to part 1 will be the number 1-jolt differences multiplied by the number of
# 3-jolt differences

# What is the total number of DISTINCT ways you can arrange the adapters to connect the charging outlet to the device
a2, x = 1, 0
l = []
...
while x < len(numbers):
    if numbers[x] + 1 in numbers:
        ...
        if len(l) > 1:
            a2 = a2 * (pow(2, len(l) - 1) - max((len(l) - 3), 0))  # The number of permutations is 2^(n-1) arrangements
        l = []
    x += 1
