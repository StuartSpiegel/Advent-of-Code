...
# currently offline, and they cannot check you in. Noticing the look on your face, they quickly add that tech support
# is already on the way! They even created all the room keys this morning; you can take yours now and give them your
# room deposit once the registration system comes back online.
#
...
# by 20201227. The card always uses a specific, secret loop size when it transforms a subject number. The door always
...
# card's public key. The door transforms the subject number of 7 according to the door's secret loop size. The result
# is called the door's public key. The card and door use the wireless RFID signal to transmit the two public keys (
# your puzzle input) to the other device. Now, the card has the door's public key, and the door has the card's public
# key. Because you can eavesdrop on the signal, you have both public keys, but neither device's loop size. The card
# transforms the subject number of the door's public key according to the card's loop size. The result is the
# encryption key. The door transforms the subject number of the card's public key according to the door's loop size.
# The result is the same encryption key as the card calculated. If you can use the two public keys to determine each
# device's loop size, you will have enough information to calculate the secret encryption key that the card and door
# use to communicate; this would let you send the unlock command directly to the door!
#
# For example, suppose you know that the card's public key is 5764801. With a little trial and error, you can work
# out that the card's loop size must be 8, because transforming the initial subject number of 7 with a loop size of 8
# produces 5764801.
#
