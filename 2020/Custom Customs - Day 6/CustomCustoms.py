# --- Day 6: Custom Customs --- As your flight approaches the regional airport where you'll switch to a much larger
...
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% In this group, there are 6 questions
# to which anyone answered "yes": a, b, c, x, y, and z. (Duplicate answers to the same question don't count extra;
...
# answers are on a single line. For example: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% abc
#
# a
# b
# c
...
with open('input.txt') as f:
    groups = [set(group.replace('\n', '')) for group in f.read().split('\n\n')]  # split the input into groups per the
    # question details that say groups are seperated by a white space (empty line)
print(sum([len(x) for x in groups]))  # for every group, people are denoted by line, so print the length of each line
# for each group and sum them

# which questions did everyone answer 'yes'
with open('input.txt') as f:
    groups = [[set(member) for member in group.split('\n')] for group in f.read().split('\n\n')]  # declare the a
    # member is each line in a group
print(sum([len(set.intersection(*member)) for member in groups]))  # the (*) operator for set.interactions() returns
# the all list entries, using the set operator you can find set intersections quickly per group per person.
