...
# language you don't understand. You should probably figure out what it says before you get to the train station
...
# understand. This ticket might be represented as 101,102,103,104,301,302,303,401,402,403; of course, the actual
# train tickets you're looking at are much more complicated. In any case, you've extracted just the numbers in such a
# way that the first number is always the same specific field, the second number is always a different specific
...
#
# For example, suppose you have the following notes: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% class: 1-3
# or 5-7 row: 6-11 or 33-44 seat: 13-40 or 45-50 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% your ticket: 7,
# 1,14 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% nearby tickets: 7,3,47 40,4,50 55,2,20 38,6,
# 12 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% It doesn't matter which position corresponds to which
# field; you can identify invalid nearby tickets by considering only whether tickets contain values that are not
# valid for any field. In this example, the values on the first nearby ticket are all valid for at least one field.
...
def part_1():
    ...
    for line in file:
        ...
        good_ranges.append([int(ranges[1].split("-")[0]), int(ranges[1].split("-")[1])])  # split the numerical ranges
        # into a list
    ...
    invalid_sum = 0  # Counter for summing invalid fields (toReturn)
    ...
    for line in file:
        ...
        for val_string in line.split(","):
            ...
            for range in good_ranges:
                if range[0] <= val <= range[1]:  # Do the actual policy check part 1 is enforcing
                    bad_val = False
                    break
            if bad_val:
                invalid_sum += val
